...
(()=>{
 ...
 let language='en';try{language=valid(localStorage.getItem(key));const requested=new URL(location.href).searchParams.get('lang');if(['en','ru'].includes(requested)){language=requested;localStorage.setItem(key,language)}}catch{}
 ...
 function numbers(s){
  ...
  if(/^\d{4}-\d{2}-\d{2}$/.test(s)){const d=new Date(s+'T12:00:00Z');if(Number.isFinite(+d))s=new Intl.DateTimeFormat('ru-RU',{day:'numeric',month:'short',year:'numeric',timeZone:'UTC'}).format(d);}
  ...
  s=s.replace(/\b(\d{1,2}):(\d{2})\s*(AM|PM)\b/g,(_,h,m,mer)=>String((Number(h)%12)+(mer==='PM'?12:0)).padStart(2,'0')+':'+m);
 ...
 function translate(source,depth=0){
  const s=normalize(String(source??''));if(language!=='ru'||!s)return String(source??'');
  let result=messages[s];
 ...
 function textNode(node){if(ignored(node.parentElement))return;const current=node.nodeValue;let entry=sources.get(node);if(!entry||current!==entry.output)entry={source:current};const output=language==='ru'?translate(entry.source):entry.source;entry.output=output;sources.set(node,entry);if(current!==output)node.nodeValue=output;}
 ...
 function set(value,persist=true){language=valid(value);document.documentElement.lang=language;document.documentElement.dataset.locale=language;if(persist){try{localStorage.setItem(key,language)}catch{};const url=new URL(location.href);if(url.searchParams.has('lang')){url.searchParams.set('lang',language);history.replaceState(history.state,'',url);}}mount();localize();localize(document.querySelector('title'));document.querySelectorAll('[data-locale-invalid]').forEach(el=>{el.setCustomValidity('');delete el.dataset.localeInvalid;});controls();document.dispatchEvent(new CustomEvent('ecocharge:locale',{detail:{language}}));}
