...
(()=>{
 ...
 downloadPdf=async function(kind){
  ...
  const station=selected?{...selected}:null,agreement=kind==='contract'?(window.EcoLocale?.html(contractContent())||contractContent()):null,confirmedLabel=station?window.EcoLocale.t(station.confirmed):'';
  try{
