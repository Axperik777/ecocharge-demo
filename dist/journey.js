...
function nextJourneyStep() {
  ...
  if (demo.planDraft) {
    ...
    return {tag:'Pick up where you left off',title:'Your plan. Saved for later.',copy:`${demo.planDraft.name} · ${demo.planDraft.stationIds.length} ${demo.planDraft.stationIds.length===1?'station':'stations'} · ${money(demo.planDraft.capital)}. Continue when you are ready.`,action:'journey-resume',label:'Continue my plan'};
  }
  if (isActivePlan()) return {tag:'Your demo plan is active',title:'Your charging workspace.',copy:`Review your ${demo.plan.stationIds.length} ${demo.plan.stationIds.length===1?'reference station':'reference stations'}, account activity and manager conversations.`,action:'journey-my-plan',label:'Open my plan'};
...
actions['journey-ask-comparison']=()=>{const rows=explorationState().compared.map(journeyStation).filter(Boolean);contactComposer('station',{entry:'station-comparison',topic:'station',stationIds:rows.map(s=>s.id),stationName:rows.map(journeyLabel).join(' / ')});$('#support-message').value=(window.EcoLocale?.language==='ru'?'Помогите разобраться в этих станциях: ':'Please help me review these reference locations: ')+rows.map(s=>`${journeyLabel(s)} (AFDC ${s.id})`).join('; ')+(window.EcoLocale?.language==='ru'?'. Какие данные об оборудовании и владении нужно проверить?':'. What equipment and ownership evidence should I check?');};
