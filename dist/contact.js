...
document.addEventListener('submit',e=>{
  ...
  if(form.id==='support-form'){
    ...
    if(isCall&&(!appointment.date||appointment.date<new Date().toLocaleDateString('en-CA')||!appointment.time)){$('#support-error').textContent='Choose a current or future date and a preferred time.';return}
    ...
    contactDraft=null;activity(`Client sent ${contactTopics[topic].toLowerCase()}`);persist();
    openModal(`<div class="success-icon">✓</div><h2>${isCall?'Callback request saved':'Your question is in the inbox'}</h2><p>${isCall?'The requested time awaits confirmation.':'Your manager can review your question and the selected details.'}</p>${contextMarkup(form._context)}<div class="modal-note">Local demo · saved in this browser. Switch to Staff console to reply.</div><button class="button primary" data-thread="${id}">View my request ${icon('arrow')}</button><button class="button secondary" data-action="contact-show-staff">Try replying as manager</button>`);
    ...
    t.messages=threadMessages(t);t.messages.push({from:'client',text,date:now});t.message=text;t.status='Open';activity('Client followed up on '+t.id);persist();showThread(t.id);
    ...
    t.messages=threadMessages(t);t.messages.push({from:'manager',text,date:now});t.reply=text;t.readReply=null;t.repliedAt=now;t.status='Resolved';activity('Manager replied to '+t.id);persist();$('#modal').close();renderAdmin();toast('Reply saved in the client inbox.');
  }else{
    ...
    const id=messageId();demo.tickets.push({id,subject,message:'',reply:text,status:'Resolved',date:now,repliedAt:now,messages:[{from:'manager',text,date:now}]});activity('Manager started conversation '+id);persist();$('#modal').close();renderAdmin();toast('Demo message is visible in the client inbox.');
  }
},true);
...
function labelMobileTables(){for(const table of $$('#admin-view .table-scroll table')){const labels=[...table.querySelectorAll('thead th')].map(th=>th.textContent.trim());for(const row of table.querySelectorAll('tbody tr'))[...row.children].forEach((cell,i)=>cell.dataset.label=labels[i]||'')}}
