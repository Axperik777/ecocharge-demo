...
function cabSettingsPanel() {
  ...
  return `<nav class="cab-settings-tabs" aria-label="Workspace settings">${[['contacts', 'Manager contacts'], ['stations', 'Station directory'], ['rates', 'Demo plans'], ['tools', 'Demo tools']].map(([id, label]) => `<button data-cab-setting="${id}" aria-current="${cabSetting === id ? 'page' : 'false'}">${label}</button>`).join('')}</nav><div class="cab-settings-content">${content}</div>`;
}
...
function cabReviewChanges(after) {
  ...
  const fields = cabEdit.kind === 'balance' ? [['balance', 'Demo balance', money]] : [['capital', 'Plan amount', money], ['tariff', 'Weekly plan', escapeHtml], ['stations', 'Selected stations', String], ['rate', 'Weekly demo rate', n => n + '%']];
