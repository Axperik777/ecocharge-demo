...
function starterCards(scope="dashboard"){
  ...
  return `<section class="life-offers" id="${scope==="dashboard"?"starter-plans":"starter-plans-page"}" aria-labelledby="starter-heading-${scope}"><div class="life-section-heading"><div><span class="j-eyebrow">THREE STARTING OPTIONS</span><h2 id="starter-heading-${scope}">Choose your starting amount.</h2></div><p>The full amount is shown up front. The company fee is separate from your plan capital.</p></div><div class="life-offer-grid">${EcoFunding.offers.map(o=>{const tier=demo.tariffs.find(t=>t.id===o.tierId);return `<article class="life-offer ${chosen?.id===o.id?'selected':''}"><span class="life-offer-label">Plan capital</span><strong class="life-capital">${money(o.capital)}</strong><p><span>Reference locations:</span> <span translate="no">${tier.count}</span> · <span translate="no">${tier.rate}%</span> <span>weekly illustration</span></p><dl><div><dt>Company fee</dt><dd>${money(o.fee)}</dd></div><div class="life-total"><dt>Total due</dt><dd>${money(o.total)}</dd></div></dl><button class="button ${chosen?.id===o.id?'primary':'secondary'}" data-starter="${o.id}" aria-pressed="${chosen?.id===o.id}"><span>${chosen?.id===o.id?'Review selection':'Choose this plan'}</span> ${icon('arrow')}</button></article>`;}).join('')}</div><p class="life-disclosure">The fee is charged once for the starting package. It is not invested and does not earn weekly credits. Calculations are illustrative; returns are not guaranteed.</p></section>`;
}
...
function salesOutline(mode){
  ...
  const price=o?`${lifeT('Plan capital')}: ${money(o.capital)}. ${lifeT('Company fee')}: ${money(o.fee)}. ${lifeT('Total due')}: ${money(o.total)}.`:lifeT('Show all three starting packages and their full prices.');
  return mode==='ftd'?[
    ['Understand the goal','What would you like to understand before choosing a starting plan?'],
    ['Explain the full price',price,true],
    ['Check understanding','The company fee is separate from plan capital. Weekly figures are illustrations, not guaranteed returns. Review the terms and withdrawal schedule together.'],
