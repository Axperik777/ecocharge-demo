...
function brand({dark=false,portal=false}={}){
  return `<a class="${portal?'brand ':''}charge-brand" href="./" aria-label="ECO CHARGE home"${dark?' data-brand-theme="dark"':''}><svg class="charge-symbol" viewBox="0 0 64 64" aria-hidden="true">${paths}</svg><span class="charge-wordmark" translate="no">ECO <span>CHARGE</span></span></a>`;
}
